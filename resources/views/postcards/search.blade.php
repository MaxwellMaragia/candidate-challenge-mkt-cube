                ...
                <div class="min-w-0 flex-1">
                    <nav class="flex" aria-label="Breadcrumb">
                        <ol role="list" class="flex items-center space-x-4">
                            <li>
                                <div>
                                    <a href="{{ url('/') }}" class="text-sm font-medium text-gray-500 hover:text-gray-700">Home</a>
                                </div>
                            </li>
                        </ol>
                    </nav>
        ...
        <main class="pt-8 pb-16">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <form class="flex group relative grid-cols-2 md:gap-4" action="{{ route('search') }}" method="GET">
                    ...
                    <button type="submit" class="bg-indigo-600 text-white text-sm leading-6 font-medium py-2 px-3 rounded-md">Search</button>
                </form>
                <p class="py-5"><b>{{ $postcards->count() }}</b> postcard/s found for keyword/s : <b>"{{ $keywords }}"</b></p>
